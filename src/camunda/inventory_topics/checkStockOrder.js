...
import { insertStockItems } from '../../graphql/mutation/insertMoOrderStockItems.js';
...
client.subscribe('checkStockOrder', {
    ...
    try {
        ...
        for (const sku of allSkus) {
            ...
            // console.log(`SKU: ${sku} | Raw Stock: ${rawStock} | Outstanding: ${outstanding} | Final Stock: ${finalStock}`);

            // Tambahan refill logic di sini
            if (finalStock <= 0) {
            ...
            for (const order of batch) {
                ...
                for (const item of items) {
                    ...
                    if (currentStock > 0) {
                        ...
                        missingStock = true;
                        const stockInfo = skuToPartId.get(skuPrefix);
                        // await insertStockItems(invoice, stockInfo.partId, quantityKonfersi, currentStock);
                    }
                    if (stockToko > 0) {
        ...
        if (uniqueRefillParts.length > 0) {
            ...
            for (const req of refillRequests) {
                ...
                const hurufKedua = name.charAt(1) || 'NotFound';
